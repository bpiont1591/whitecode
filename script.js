...
const opinionStatusEl = document.getElementById("opinionStatus");
const opinionSubmitBtn = opinionForm?.querySelector('button[type="submit"]');
...
function setOpinionStatus(msg, type) {
  if (!opinionStatusEl) return;
  opinionStatusEl.textContent = msg;
  opinionStatusEl.className = "status status-box " + (type || "");
}
...
function setOpinionSubmitting(isSubmitting) {
  if (!opinionForm) return;
  const controls = opinionForm.querySelectorAll("input, textarea, select, button");
  for (const ctrl of controls) {
    ctrl.disabled = isSubmitting;
  }

  if (opinionSubmitBtn) {
    opinionSubmitBtn.classList.toggle("is-loading", isSubmitting);
    if (isSubmitting) {
      opinionSubmitBtn.dataset.originalText = opinionSubmitBtn.innerHTML;
      opinionSubmitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Zapisuję...';
    } else if (opinionSubmitBtn.dataset.originalText) {
      opinionSubmitBtn.innerHTML = opinionSubmitBtn.dataset.originalText;
    }
  }
}
...
function setAuthUI(user) {
  ...
  if (opinionSubmitBtn) opinionSubmitBtn.disabled = !isAuth;
  ...
  if (isAuth) {
    ...
    if (authInfoEl) authInfoEl.textContent = "Wymagane logowanie przez Discord przed wysłaniem wiadomości i opinii.";
...
async function submitOpinion() {
  if (!opinionForm) return;
  if (!loggedUser) {
    setOpinionStatus("Najpierw zaloguj się przez Discord.", "err");
    return;
  }

  const rating = Number(document.getElementById("opinionRating")?.value || 0);
  const atmosfera = String(document.getElementById("opinionAtmosfera")?.value || "").trim();
  const przebieg = String(document.getElementById("opinionPrzebieg")?.value || "").trim();
  const text = String(document.getElementById("opinionText")?.value || "").trim();

  if (!rating || !atmosfera || !przebieg || !text) {
    setOpinionStatus("Uzupełnij wszystkie pola opinii.", "err");
    return;
  }

  setOpinionStatus("Zapisuję opinię...", "");
  setOpinionSubmitting(true);

  try {
    const res = await fetch(API_OPINIONS, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rating, atmosfera, przebieg, text })
    });

    const out = await res.json().catch(() => ({}));
    if (!res.ok || !out.ok) {
      setOpinionStatus(out.error || "Nie udało się zapisać opinii.", "err");
      return;
    }

    opinionForm.reset();
    setOpinionStatus("Opinia została dodana ✅", "ok");
    await refreshOpinions();
  } catch {
    setOpinionStatus("Błąd połączenia. Spróbuj ponownie.", "err");
  } finally {
    setOpinionSubmitting(false);
    if (opinionSubmitBtn && !loggedUser) opinionSubmitBtn.disabled = true;
  }
}
